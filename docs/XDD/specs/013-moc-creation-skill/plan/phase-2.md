...
- [x] **T2.1 CLI scaffolding + mode routing** `[activity: backend-cli]` ✅ DONE 2026-05-08: `tomo/scripts/moc-discovery.py` v0.1.1 + `tests/test_moc_discovery_cli.py` v0.1.1 (9 tests passing). argparse with mutex group, route_input() with strict `is not None` + non-empty validation, Phase 1-6.5 NotImplementedError stubs, full DiscoveryReport JSON shape. Commits: `f587ccc` + `9cda724`.
...
- [x] **T2.2 Phase 1 — Candidate selection (5 modes + pre-filter + caps)** `[activity: backend-discovery]` ✅ DONE 2026-05-08: `tomo/scripts/moc-discovery.py` v0.2.0 + `tests/test_moc_discovery_phase1.py` v0.1.0 (8 phase1 tests + 9 prior CLI tests = 17 passing). `Candidate` dataclass, 5 mode handlers (`_handle_tag` with `*` glob suffix per ADR-7, `_handle_folder` with client-side `.md` filter per ADR-6, `_handle_class` resolving Dewey subdirs via profile, `_handle_title_or_freetext` matching `cache.map_notes[].topics`, `_handle_scan` over atomic-note subdirs), `restrict_to_atomic_note_paths` pre-filter with stderr WARN on intersection, `phase1_select_candidates` returning `(candidates, abort_reason)` with `zero-candidates` / `candidate-cap-exceeded` short-circuit.
