...
#### Feature 7: Scan output-quality cleanup — notes-only default, bounded output, on-demand MOC-uplink check
- **User Story:** As the vault owner, when I run a whole-vault `/moc-propose` scan I want a usable, bounded result: links for atomic notes that lack a MOC home (most actionable first), not a flood, and not polluted with template-vault MOCs or root maps that legitimately have no parent. When I specifically want to audit MOC parentage, I want a separate `check:` mode for that.
- **Context (live, 2026-06-06):** A plain scan emitted **251 orphan suggestions** (206 notes + 45 MOCs). The 45 MOCs were noise — 17 from the `X/` template-vault (Travelguide/TemplateVault/Readwise), 17 Efforts MOCs, and 6 root maps (`000 Index` etc.) which are top-level and correctly have no parent. The 206 notes were verified **real** (Kado-sampled, `lib/up_parse` correct: empty `up::` placeholders + notes with no `up` field) — a genuine vault state, not a defect, so the fix is *output shaping*, not parser change.
- **Acceptance Criteria (Gherkin Format):**
  - [ ] Given a whole-vault `scan`, When the orphan link-or-create pass runs, Then only `kind == "note"` orphans are emitted by default (MOC-kind orphans are NOT in the default scan output).
  - [ ] Given the ordered orphan set exceeds `tomo.moc_proposal.orphan_display_cap` (default 50), When the proposal-doc is rendered, Then only the first `orphan_display_cap` suggestions appear And an overflow footer states how many more were omitted and to re-run with a scoped query.
  - [ ] Given orphans of both kinds of outcome, When they are ordered for display, Then `mode == "link_existing"` suggestions appear before `mode == "create_new"` ones (most-actionable first), link_existing sub-sorted by top-candidate score DESC.
  - [ ] Given `/moc-propose check:moc-uplinks`, When it runs, Then the orphan pass runs over `kind == "moc"` entries only (a focused MOC-parentage audit) And the clustering pipeline (Phase 1–6) is skipped.
...
- **M2 — Placeholder false-positive drop:** _Re-baselined on the clean vault 2026-06-09 (T4.3)._ Two corrections compound. (a) **Detection** (`lib/placeholder_detect`): dead-link count on the real vault drops **397 → 196 unique** — the ~224 targets that resolve to an existing in-scope note are removed (224-fix), plus 23 date-shaped periodic-note links (`[[2022-06-10]]`) that resolve to real daily notes outside MOC scope. `placeholder.build` telemetry (`raw_count`/`kept_count`/`false_positive_dropped`) is emitted to stderr by `moc-tree-builder` for this. (b) **Condition C feed** (`shared-ctx-builder.build_placeholder_links`): of the 196, only those following the vault's MOC naming convention (`(MOC)` / ` MOC`) are passed to Condition C — **38 missing-MOC placeholder links** on the clean vault; the other ~158 are missing *regular* notes, not missing MOCs, and must not trigger MOC creation. _Terminology: the cache holds **placeholder links** (all dead wikilinks); only the MOC-convention subset are **missing MOCs**. The original "~171 / ≤180" target was a Jun-5 estimate on a different snapshot and is superseded — the fix is validated by the drop (197 FPs removed at detection) and the 196→38 MOC-convention narrowing, not an absolute ceiling._
...
- **M4 — Tag-primary recovers notes-area MOCs:** the previously-missed `#type/others/moc` notes are recognised as MOCs and removed from the placeholder set. _(2026-06-09: this is now distinct from the Condition C MOC-naming filter in M2(b) — M4 removes notes that ARE tagged MOCs from the dead-link set; M2(b) restricts which dead links are treated as MOC-creation candidates by name convention.)_
