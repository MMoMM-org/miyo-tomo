...
| F-43 | MOC-creation skill — proactive MOC propose / create outside inbox | 2026-05-06 Obsidian-power brainstorm | **Must — PRD draft 2026-05-07** | **Status: XDD 014 PRD drafted** at `docs/XDD/specs/014-moc-creation-skill/` 2026-05-07. SDD + plan deferred — see PRD §8 for OQ1–OQ8 still requiring stakeholder input. Standalone workflow to propose and materialise MOCs by topic-area, separate from the inbox-driven trigger logic in F-34/F-35. Two commands: `/moc-propose <topic>` (scan vault for cluster candidates, suggest MOC-name + structure) and `/moc-create` (materialise an approved proposal — file with templated sections, bidirectional `up:`/`down:` on children, profile-aware naming). Profile-aware: miyo (Dewey 2000-2900 ranges, MOC-only at major class) vs LYT (free thematic). Reuses existing `create_moc` + `add_relationship` schema actions; reuses suggestions-doc UI; reuses Hashi executor. New: `moc-architect.md` agent + `moc-propose.py` / `moc-render.py` scripts. Reference-skill prereq: import `obsidian-markdown` during this track. Roadmap position: track #1 in `roadmap-obsidian-power.md`. Launch gate satisfied 2026-05-07 (commit `053bc4e`). Will supersede F-13 (`/scan-mocs`). |
