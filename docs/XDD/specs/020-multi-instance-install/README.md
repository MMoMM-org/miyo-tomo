...
| 2026-06-01 | Findings logged to backlog | **D-09 closed** by ADR-7 (`render-launcher.sh`). **D-11 raised** — `cleanup-tomo.sh` (v0.1.0) is not multi-instance-aware (operates on `$REPO_ROOT/tomo-instance/`, refuses external paths); its docs left unchanged. (D-10 = documentation-refresh, already reserved on branch `docs/backlog-d10-documentation-refresh`.) **D-06 reinforced** — tier-2/tier-3 architecture-reference docs still describe the old `#MiYo-Tomo/<state>` tag model; deliberately left out of this spec's *user-facing* sweep, to fold into the D-10 docs refresh. |
