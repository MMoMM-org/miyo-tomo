...
| F-43 | MOC-creation skill — proactive MOC propose / create outside inbox | 2026-05-06 Obsidian-power brainstorm | **Must — Spec ready (PRD+SDD+PLAN) 2026-05-07** | **Status: XDD 013 spec complete** at `docs/XDD/specs/013-moc-creation-skill/` 2026-05-07. PRD (8 features, 35 Gherkin AC, 4 deferred-to-SDD questions) + SDD (9 ADRs, 1234 lines, additive 2-pass extension) + PLAN (6 phases, 29 tasks, 9 parallel-eligible, TDD throughout). Validation passed (3 minor findings patched). Brainstorm spec at `docs/XDD/ideas/2026-05-06-moc-creation-skill.md`. Standalone workflow with `/moc-propose <topic>` (proactive cluster discovery) and `/moc-create` (materialise via existing `create_moc` + `add_relationship` schema actions through Hashi). Profile-aware (miyo Dewey vs LYT thematic). New components: `moc-architect.md` agent + `moc-discovery.py` script; extensions to suggestions-reducer/parser/render and inbox-analyst Step 2b. Roadmap position: track #1 in `roadmap-obsidian-power.md`. Launch gate satisfied 2026-05-07 (commit `053bc4e`). Will supersede F-13 (`/scan-mocs`). Implementation pending — branch `feat/f-43-moc-creation-skill` when picked up. |
