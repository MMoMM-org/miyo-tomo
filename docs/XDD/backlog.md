...
| F-33 | Force Atomic Note synthesis via follow-up suggestions doc | Pass-2 live run 2026-04-23 (Furano case) | Should | When Pass 2 parses a Force Atomic Note checkbox but the analyst emitted no per-item section for that item, today the parser skips the atomic creation with a warning (see commit `2665f81`). Desired UX: Pass 2 should instead generate an **additional suggestions doc** containing a newly-proposed atomic-note suggestion for that item, then **halt** without rendering. On the next `/inbox` run, Pass 2 sees two suggestion docs present — detects that the Force Atomic Note in doc A now has a matching atomic-note suggestion in doc B — and renders correctly once the user has approved doc B. Keeps proposal-first (no silent synthesis) and closes the UX gap where FAN is unusable on sub-worthiness items. Architecture impact: Pass 2 gains a mini-Pass-1 LLM hop (single-item atomic proposal), multi-doc reconciliation, and a "halt-waiting-for-user" state that persists across /inbox runs. Worth speccing as its own XDD once prioritised. |
