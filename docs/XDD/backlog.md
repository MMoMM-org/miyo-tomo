...
| F-47 | Tomo lifecycle tags — unified frontmatter + byTag discovery for all Tomo-produced docs | 2026-05-20 F-43 T6.2 live-validation findings | **Must — PRD in progress** | **Status: XDD 017 in PRD phase** at `docs/XDD/specs/017-tomo-lifecycle-tags/`. Three production gaps surfaced during F-43 live-vault validation: (1) `/inbox` Auto-Discovery does `listDir + 3× kado-read` of full file bodies to count markdown checkboxes for state detection — high token cost; (2) `tomo-moc-proposal-*` not in `state-init.py` SKIP_SUFFIXES (filename uses prefix not suffix) so proposal-docs are seeded as source items; (3) state is split across three mechanisms (filename-suffix, markdown-checkboxes in body, frontmatter tag `#<prefix>/captured`) — no unified model. **Scope**: introduce `#tomo/<doc-type>/<state>` hierarchical tag + `tomo:` frontmatter block (run_id, doc_type, state, updated_at) on every Tomo-produced doc. State machines per doc-type (source: pending→captured; suggestions: pending-approval→approved→converted; moc-proposal: pending-accept→accepted→rejected; instructions: pending-apply→applied). Discovery flow switches to `kado-search byTag #tomo` + per-file `read_frontmatter` (no body reads). Filename rename `tomo-moc-proposal-*` → `YYYY-MM-DD_HHMM_moc-proposal-<slug>.md` for consistency with other Tomo outputs. Backward-compat fallback for legacy files (no `tomo:` block) keeps the suffix+checkbox path until next major release. **Migration phases**: P1 producer-side writes, P2 byTag consumer + state-init refactor, P3 filename rename, P4 MOC-consumption flow (closes F-43 acceptance gap), P5 legacy-fallback removal. **Hard blocker for**: F-43 T6.2/T6.4 launch; future F-44/F-45/F-46 will inherit the unified model so consistency matters. Touch points: `state-init.py`, `inbox.md` Auto-Discovery, `inbox-orchestrator.md` Phase A, `suggestions-render.py`, `instruction-render.py`, `suggestions-reducer.py` (--moc-proposal-mode), `tag-captured.py`, `vault-executor`, `suggestion-parser._is_moc_proposal_doc`, new `tomo/schemas/doc-frontmatter.schema.json`. |
