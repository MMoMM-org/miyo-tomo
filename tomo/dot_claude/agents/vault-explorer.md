...
**If the scanner fails:** Surface its stderr output to the user. Run cache-builder WITHOUT
`--accumulation` so the cache is still generated with an empty accumulation index — do not
abort the step entirely.
...
- **Accumulation:** `accumulation_cluster_count` unclassified topic clusters
- **Cache:** `cache_path`
