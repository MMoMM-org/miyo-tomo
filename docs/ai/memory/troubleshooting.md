...
## Tomo emitted an `add_relationship.error` field Hashi rejects → whole instruction set un-appliable — Status: resolved (commit `c893251`)

When a confirmed MOC up-link's child note was missing or non-markdown at Pass-2 render, `instruction-render.py` (`emit_up_preservation_actions`) emitted an `add_relationship` action carrying `error: "child-missing"` / `"non-markdown-asset"` (`applied: False`), and it flowed **unfiltered** into the wire `instructions.json` `actions`. Hashi's schema is `additionalProperties:false` with no `error` field, so Hashi rejects the **ENTIRE** instruction set — one missing child note breaks the whole batch at apply. Fix: `filter_unappliable_relationships(actions) -> (kept, skipped)` (mirrors `filter_missing_daily_notes`) drops error-bearing actions **before serialization** (and before `_validate_action_paths`) and surfaces them in the instructions.md "Skipped — un-appliable actions" section; `error` removed from the producer schema. Durable rule (recurring): **Tomo must never put on the wire a field or un-appliable action that Hashi's `additionalProperties:false` schema would reject — filter un-appliable actions (and producer-internal fields) before serialize.** Same class as "Hashi modifies, never creates" (auto-memory `domain_hashi_modifies_never_creates`): an action Tomo already knows can't be applied should be surfaced as Skipped, not sent. Found by the new bidirectional schema-parity guard (see `decisions.md` 2026-06-24), 2026-06-24.
