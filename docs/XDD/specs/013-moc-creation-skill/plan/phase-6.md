...
- [ ] **T6.2 Live-vault validation on Marcus's real vault** `[activity: e2e-live]` 🔶 PAUSED 2026-05-20 — blocked-by F-47 (XDD 014 Tomo Lifecycle Tags). Partial-run findings before pause:
  ...
  - **Resume condition**: F-47 P1 + P2 shipped (producer-side frontmatter writes + consumer-side byTag discovery). At that point `/moc-propose` proposal-docs carry `#tomo/moc-proposal/pending-accept` and `/inbox` can route them via the unified acceptance-flow. Then re-run T6.2 covering remaining modes (folder/class/title/free-text/scan) + Override + collision-guard against real vault.
