...
- [x] **T1.1 `lib/profile_conventions.py` resolver** `[activity: domain-modeling]`
  ...
  5. Success:
     - [x] Both bundled profiles resolve correct suffix (miyo `" (MOC)"`, lyt `""`) `[ref: PRD/AC F-55]`
     - [x] Missing keys fall back to documented defaults, no crash `[ref: PRD/Config & fallback]`
     - [x] `profiles_dir` required (ADR-2) `[ref: SDD/ADR-2]`

- [x] **T1.2 Profile `name_suffix` keys + version bumps** `[activity: data-architecture]`
  ...
  5. Success:
     - [x] `map_note.name_suffix` present + consumed in both profiles `[ref: PRD/AC F-55/Feature 3]`
     - [x] `# version:` bumped on both (else `update-tomo` ships nothing) `[ref: SDD/CON-5]`
