...
- [x] **T6.2 Live-vault validation on Marcus's real vault** `[activity: e2e-live]` 🔶 DONE-pending-operator 2026-05-21 — structural work complete in F-47 Phase 5 (T5.1+T5.2+T5.3); final E2E live-vault validation deferred to operator per F-47 evolution log. Done in F-47 Phase 5 T5.1+T5.2+T5.3; live launch-gate validation deferred to operator per `tomo/docs/evolution/2026-05/2026-05-21_F-47-P4-moc-consumption-launch-gate.md`. Earlier PAUSED 2026-05-20 — blocked-by F-47 (XDD 014 Tomo Lifecycle Tags). Partial-run findings before pause:
  - **Mode 1 (tag)** ran end-to-end successfully against `topic/knowledge/lyt`: 34 raw → 30 in-scope candidates → 8 clusters → proposal-doc rendered + transported to vault (`100 Inbox/tomo-moc-proposal-20260520-1359-notemaking-moc.md`). 5 clusters in doc + 3 overflow (emergence-levels-moc, pkm-personality-moc, architects-gardeners-moc). Performance not measured (T6.5/T6.5.5 fixes consumed the timing budget).
  - **Two production bugs surfaced + fixed** during this run: T6.5 (agent-side topic extraction, agent v0.2.0 + script v0.10.0) and T6.5.5 (agent kado-write transport, agent v0.3.0). See README decision log + commits `17d5a00`, `6e66d65`, `42e4d99`. Both fixes live; tag-mode now hits the happy path without manual recovery.
  - **Architectural blocker discovered**: `/inbox` Auto-Discovery does listDir + 3× kado-read of full file bodies to detect state (checkbox-counting). `tomo-moc-proposal-*` is not in `state-init.py` `SKIP_SUFFIXES`. No `tomo:` frontmatter / hierarchical lifecycle tag on Tomo-produced docs. Acceptance-flow for the proposal-doc (`/inbox` consumes accepted clusters → emits `create_moc` + `link_to_moc` instructions) cannot be validated without the unified discovery model. New spec **`017-tomo-lifecycle-tags`** (F-47) takes the refactor — see `docs/XDD/specs/017-tomo-lifecycle-tags/` for PRD and `docs/XDD/backlog.md` F-47 entry.
  - **Resume condition** (updated 2026-05-21 against F-47 PRD v1.2 + SDD v1.0 phase structure):
    - **T6.2 remaining modes (folder/class/title/free-text/scan) + Override + collision-guard** — resume after **F-47.P2** ships. P2 delivers unified byFrontmatter discovery so `/inbox` sees the proposal-doc; P1 already gives the proposal-doc a `tomo:` block (`doc_type=moc-proposal`, `state=pending-accept`). The remaining modes test proposal-doc *creation + discovery*, not consumption.
    - **T6.4 final launch gate (incl. accepted-cluster end-to-end test)** — resume after **F-47.P4** ships. P4 delivers the bundled MOC-consumption flow (`create_moc` + N× child-relationship updates in one instructions doc); T6.4's accept-flow test depends on P4 being live.
    - Earlier note said "P1 + P2" — superseded by F-47 v1.2 phase structure. See `docs/XDD/specs/017-tomo-lifecycle-tags/solution.md` §Cross-Spec Coordination for the full coordination map.
...
- [x] **T6.4 Final Phase Validation & Launch Gate** `[activity: validate]` DONE-pending-operator 2026-05-21 — Done in F-47 Phase 5 T5.1+T5.2+T5.3; live launch-gate validation deferred to operator per `tomo/docs/evolution/2026-05/2026-05-21_F-47-P4-moc-consumption-launch-gate.md`.
