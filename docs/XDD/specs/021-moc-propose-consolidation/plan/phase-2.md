...
- [x] **T2.1 `lib/moc_cache_loader.py` — TTL + rebuild-if-stale + shim** `[activity: backend-api]` `[ref: SDD/Application Data Models loader shim; ADR-1,3,8]`
...
- [x] **T2.2 moc-discovery Phase 6.5 uses `up_parse` (frontmatter + inline)** `[activity: backend-api]` `[ref: SDD/ADR-6; SDD C1; PRD/Feature 2]`
...
- [x] **T2.3 `lib/orphan_link.py` — case (a) orphan pass (notes AND MOCs)** `[activity: backend-api]` `[ref: SDD/Implementation Examples case-(a); SDD ADR-7 (H2/H3); OQ-4; PRD/Feature 3]`
