...
| 2026-06-29 | Live-walk bug fixed: `path_exists` `.md`-only → extension-routed | The `.base` live walk surfaced a real defect: `kado-write-file.py --no-overwrite` errored (exit 1) for every non-`.md` artifact because `KadoClient.path_exists()` probed via `read_frontmatter`, which Kado rejects on non-`.md` paths with VALIDATION_ERROR (not NOT_FOUND) — so the collision guard was broken for exactly the `.base`/`.canvas` formats this spec added (the session fell back to an off-pipeline direct `kado-write`). Fixed: `path_exists()` routes the probe by extension (`.md`→frontmatter, non-`.md`→`read_file`); regression test `test_kado_client_path_exists.py`; kado_client 0.8.0→0.9.0, kado-write-file 0.3.1→0.3.2; instance re-synced. The T1.2 unit test missed this by mocking `path_exists` instead of hitting the real `.md`-only path. |
| 2026-06-29 | Live walk passed — all 22 PRD ACs met | Per-format `.md`/`.base`/`.canvas` Compose-to-Inbox walk run by the user in the live Tomo session: all green after the `path_exists` extension-routing fix. Last open acceptance item closed; spec fully verified end-to-end. |
