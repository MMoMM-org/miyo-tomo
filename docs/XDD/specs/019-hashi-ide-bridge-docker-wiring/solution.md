...
- [x] **ADR-3 — reachability probe**: Detect Hashi reachability with a **TCP-connect test** to `127.0.0.1:<port>` (bash `/dev/tcp` or `nc -z`) — host-side in `begin-tomo.sh`, container-side (via the proxy) in the statusline. Not a full WS handshake, not lock-file-presence-only.
  ...
  - User confirmed: Yes (2026-05-28)
  - **Amendment (2026-06-12, #48)**: the container-side decision "via the proxy" was wrong and produced a false green. Inside the container `127.0.0.1:<port>` is the socat listener, which `TCP-LISTEN,fork` accepts unconditionally *before* its upstream `connect()` — so the statusline probe was green whenever socat ran, regardless of Hashi, while socat spammed `E connect(...) Connection refused`. Fixes: (a) the **container statusline probe now targets `host.docker.internal:<port>` directly** (the real host upstream), bypassing the proxy, so green means Hashi is genuinely reachable; (b) this introduces a dependency on `--add-host=host.docker.internal:host-gateway` on the `docker run` line so the name reliably resolves inside the container (it was previously unset and only resolved on Docker Desktop); (c) socat stderr is redirected to `~/.claude/ide/socat.log` so a dead upstream no longer floods the session. The **host-side banner probe is unchanged** — on the host loopback there is no socat, so a TCP accept there genuinely means Hashi is listening.
