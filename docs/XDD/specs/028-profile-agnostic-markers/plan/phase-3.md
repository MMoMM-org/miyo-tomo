...
- [x] **T3.1 up_parse parse-by-param** `[activity: backend]` `[parallel: true]`
...
- [x] **T3.2 render_actions read+write markers** `[activity: backend]`
  ...
  5. Success:
     - [x] miyo rendered actions byte-identical `[ref: PRD/AC F-16; SDD/Quality Requirements]`
     - [x] markers sourced from conventions on read + write `[ref: PRD/Feature 1]`

- [x] **T3.3 moc-discovery up:: warning regex** `[activity: backend]` `[parallel: true]`
...
- [x] **T3.4 suggestion-parser override header from conventions** `[activity: backend]`
  ...
  5. Success:
     - [x] header detection uses profile marker `[ref: PRD/Feature 1]`
     - [x] graceful fallback when block absent `[ref: PRD/Config & fallback]`
