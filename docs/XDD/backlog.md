...
> Updated: 2026-05-06 (F-42–F-46 added during Obsidian-power brainstorm; see `roadmap-obsidian-power.md` for sequencing).
...
| F-34 | Mental Squeeze Point — Condition B: Accumulation Detection | AUDIT_REPORT_2026-04-18 patch 2026-04-23 (M2, HIGH) | **Must** | Spec (Tier-3 New MOC Proposal §2 Condition B): when a current item's topics match 2+ existing notes with no MOC link (`up::` absent), propose creating a new MOC. Today this trigger does not fire end-to-end: `tomo/dot_claude/agents/inbox-analyst.md` v0.8.0 explicitly forbids `kado-search` (lines 50, 466), and there is no orchestrator-side accumulation index in shared-ctx. Implementation options: (a) add `kado-search` to inbox-analyst tool list + Condition B logic in Step 8 (cheaper to implement, but adds vault-search per item — pricey at batch); (b) pre-compute an accumulation index in `shared-ctx-builder.py` (one search per known unclassified topic-cluster, then per-item match is a dict lookup — cheaper at batch, more upfront work). Decision needed before implementation. **MVP risk:** introducing kado-search calls per item could regress Pass-1 latency/cost; option (b) keeps Phase-B's cost profile unchanged. |
| F-35 | Mental Squeeze Point — Condition C: Placeholder MOC trigger | AUDIT_REPORT_2026-04-18 patch 2026-04-23 (M3, MEDIUM) | **Should** | Spec (Tier-3 New MOC Proposal §2 Condition C): when an item's topics match a `placeholder_mocs[]` entry (a wikilink target with no backing file), propose creating that MOC to resolve the dead link. Detection works (`tomo/scripts/moc-tree-builder.py:464-504`, output at `:676`) and caching works (`tomo/scripts/cache-builder.py:337` writes `placeholder_mocs[]` to cache.json). **Gap:** `tomo/scripts/shared-ctx-builder.py` does not surface `placeholder_mocs` — zero references in 494 lines. Phase-B never sees the list. Two-line fix: include `placeholder_mocs[]` in shared-ctx output (or a topic-keyed index of it); add a Condition C trigger note in `inbox-analyst.md` Step 8. Low MVP risk — purely additive to shared-ctx envelope (subagent ignores fields it doesn't reference). |
...
| F-41 | Multi-topic detection for atomic-note-worthy items | 2026-05-01 voice-transcript review (Apothekerpfädchen 11 case) | Should | Spec gap (Tier-3 atomic-note classification): when an inbox item carries multiple distinct concepts that each pass the worthiness gate, the analyst today emits at most ONE `create_atomic_note` action. Concrete failure (2026-05-01 /inbox run): a 183-sec voice memo combined a medical-appointment thread (daily-log relevant) with a Tomo/PKM-architecture thread (atomic-note worthy). Analyst emitted only `update_daily` — the PKM thread was lost. Desired behaviour: detect topical splits in a single item, emit one `create_atomic_note` per substantive concept (with topic-scoped suggested titles, individual MOC matches, and per-topic `tags_to_add`). Architecture impact: Step 7+8 grow a topical-segmentation pass; suggestions-reducer + parser must accept multiple atomic-note actions per source item; the FAN resolve subflow may need a 1:N mapping. Touch points: `inbox-analyst.md` Step 7 (worthiness now per-topic), Step 8 (multi-action emission), `suggestions-reducer.py` per-item rendering, `suggestion-parser.py` action-list handling, possibly `instructions.schema.json`. Worth speccing as its own XDD when prioritised. **Workaround until shipped:** user ticks Force Atomic Note on log entries to trigger the FAN resolve subflow (XDD 012) — but FAN currently produces only one atomic per item, so multi-topic items still lose threads. |
