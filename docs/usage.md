...
- **Slash commands** for structured workflows (`/inbox`, `/explore-vault`, `/execute`, `/tomo-setup`, `/tomo-help`).
...
4. **Apply the actions.** Tomo doesn't write outside the inbox; this step is yours. Two paths:
    ...
    Run `/execute` in-session if you want a path-picker reminder with the file path and action counts pre-filled. `/execute` is read-only — it never applies actions itself.
5. **Cleanup.** Re-run `/inbox`. When it sees an instruction set with all actions applied (whether by hand or by Hashi), it dispatches the vault-executor agent: source items transition from `<prefix>/captured` to `<prefix>/active`, and completed workflow docs are archived per your vault-config rules.
...
When you approve a suggestions doc and Tomo runs Pass 2, the result is an **instruction set** — a markdown document with concrete `write_file`, `link`, `tag`, `archive` actions. You apply those actions one of two ways: by hand in Obsidian (write the new note, add the link, set the tag, tick `[x] Applied`) or via [Tomo Hashi](https://github.com/MMoMM-org/miyo-tomo-hashi)'s instruction-set executor with your chosen preview mode. Run `/execute` in-session for a path-picker reminder. Either way, the next `/inbox` run sees the fully-applied instruction set and dispatches the vault-executor agent to handle cleanup: lifecycle transitions on source items and archive of completed workflow docs.
...
| You apply the instruction set | Each action is applied — manually in Obsidian (tick `[x] Applied` per action) or via Tomo Hashi's instruction-set executor. Run `/execute` for a path-picker reminder. |
| `/inbox` cleanup | The next `/inbox` run dispatches vault-executor: source items transition `<prefix>/captured` → `<prefix>/active`; fully-applied workflow docs are archived per your vault-config rules. |
...
| `/execute` | Helper that shows the two paths for applying an instruction set: manual in Obsidian, or via Tomo Hashi's instruction-set executor. Read-only — never applies actions itself. | (none) |
