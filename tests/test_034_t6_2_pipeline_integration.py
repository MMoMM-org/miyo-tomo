...
Two siblings were written alongside this file for T6.2 and live apart from it,
because neither is about subfolder discovery:

  - `tests/test_pass2_flat_instruction_golden.py` guards every future Pass-2
    change against a recording of that chain at `ee44cb3`
  - `tests/test_034_feature_coverage_map.py` maps all ten PRD features to the
    tests that carry them, across seven files and every phase of this spec

Three of that map's rows point back into this file.
