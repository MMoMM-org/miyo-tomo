...
WHY (section-read heading strip, SKILL 0.2.3, live-walk fix 2026-06-26): the target-section read in step 2 must pass the marker's HEADING TEXT to kado-read (`heading: "Captures"`), NOT the `##`-prefixed marker (`"## Captures"`). Kado's `mode:section` matches the heading text without the leading `#` run — passing `"## Captures"` returns NOT_FOUND, so the helper signals `no_structure_under_marker` and the row silently degrades to a prose fallback even though the section exists. Mirrors `_marker_to_anchor_value` on the render side, which already strips the `#` run.
