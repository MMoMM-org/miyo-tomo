...
def test_child_missing_at_render_time() -> None:
    """Kado raises NOT_FOUND → action list contains one applied:false entry.
    ...
    action dict with applied=False and error="child-missing".  Other children
    in the cluster are unaffected (no exception escapes the function).
    """
