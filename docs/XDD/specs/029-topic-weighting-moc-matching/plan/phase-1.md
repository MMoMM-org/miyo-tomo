...
- [ ] **T1.1 `topic_match` scorer + unit suite** `[activity: domain-modeling]`
  ...
  2. Test (RED) — write `tests/test_topic_match.py` first, all failing:
     ...
     - `test_misfire_incidental_overlap_rejected` — title themes disagree, only a content keyword shared → score `< 0.80` `[ref: PRD/AC Feature 1 misfire]`
     - `test_true_dup_title_agreement_survives` — shared title themes → score `≥ 0.80` `[ref: PRD/AC Feature 1 no-regression]`
     ...
     - `test_weight_is_capped_regardless_of_title_length` — long title does not distort beyond `W_TITLE` `[ref: PRD/Edge Cases long-title]`
  ...
  5. Success:
     - [ ] All six tests pass `[ref: PRD/Feature 1 + Edge Cases]`
