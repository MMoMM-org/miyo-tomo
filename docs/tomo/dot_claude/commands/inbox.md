...
`--force` is the separate, explicit "redo everything" escape hatch: it ignores coverage and drift entirely and re-synthesizes all approved docs, AND folds already-`captured` source items back into `new_sources` so Pass 1 re-suggests them too. The split exists because predictable recovery must be autonomous — the user runs `--pass2` for the common "just redo what I changed" case and `--force` only when they truly want a full rebuild, without Tomo having to ask which one they meant.
