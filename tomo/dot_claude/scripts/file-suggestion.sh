...
#
# Unified picker — no scope prefixes. Applies the rg-snippet pattern (all
# candidates → fzf --filter → head 15) to the Kado-backed data sources.
#
# Model:
#   @          → open notes + ALL inbox + ALL vault (head 15)
#   @<query>   → same candidate set → fzf --filter "<query>" → head 15
#
# Design notes:
#   - Empty @ still shows open notes first (active-note context wins via
#     emission order + awk !seen dedupe).
#   - Partial query drills across all sources with one fzf pass.
#   - No `inbox/` / `vault/` prefix syntax — earlier designs had it but
#     selecting `inbox/` inserted `@"inbox/"` or `@inbox/ ` (with space),
#     forcing the user to backspace. The unified stream avoids that.
#   - Strict substring fallback when fzf is unavailable.
#
# Why paths from Kado (vault-relative) don't resolve locally:
#   kado-open-notes returns vault-relative paths like `Calendar/301 Daily/
#   2026-03-26.md`. Claude Code's @-resolver tries Read → ENOENT → the
#   calling Claude session falls back to kado-read. One extra tool call
#   per pick. Documented in rules/project-context.md.
#
# Bash 3.2 compatible. Always exits 0 (non-zero hides the picker silently
# — confirmed T1.1 spike). Per-keystroke invocation; script is re-read
# from disk each time so no Tomo restart is needed after updates.
