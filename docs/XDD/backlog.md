...
| F-47 | Tomo lifecycle state — unified frontmatter block + byFrontmatter discovery for all Tomo-produced docs | 2026-05-20 F-43 T6.2 live-validation findings | **Must — PRD v1.2 in review** | **Status: XDD 017 PRD v1.2** at `docs/XDD/specs/017-tomo-lifecycle-tags/requirements.md`. v1.1 (2026-05-21) reframed discovery to `kado-search operation=byFrontmatter` (Kado 0.10.0/0.11.0 — paths + frontmatter inline in one call) with `filter.path` server-side narrowing. **v1.2 (2026-05-21) dropped the mirrored lifecycle tag** — user doesn't browse via Obsidian tag pane and hides frontmatter in editor, so the tag added write payload + drift-handling without UX benefit. Single SoT field is now `tomo.state`. **Scope locked in v1.2**: structured `tomo:` block (`doc_type`, `state`, `run_id`, `updated_at`, `source_*` refs) on every Tomo doc; state machines per doc-type; sequential multi-pending state-promotion; drift detection + `/inbox --recover`; transcription stop-gate; bundled MOC actions; Hashi cleanup contract state-driven and doc-type-agnostic (iterates `tomo.source_*` generically). **Migration phases**: P1 producer-side writes + write_frontmatter wrapper, P2 byFrontmatter consumer + state-init refactor, P3 drift + transcription gate, P4 MOC-consumption flow (closes F-43 acceptance gap), P5 schema-lock handoff to Hashi. **Hard blocker for**: F-43 T6.2/T6.4 launch; F-44/F-45/F-46 will inherit the unified model. Touch points: `state-init.py`, `inbox.md` Auto-Discovery, `inbox-orchestrator.md` Phase A, `suggestions-render.py`, `instruction-render.py`, `suggestions-reducer.py` (--moc-proposal-mode), `tag-captured.py` (rename or repurpose — see Feature 6 AC-6.2), `vault-executor`, `suggestion-parser._is_moc_proposal_doc`, new `tomo/schemas/doc-frontmatter.schema.json`, `kado_client.write_frontmatter` wrapper. |
