...
- [ ] **T2.1 moc-discovery suffix from profile** `[activity: backend]`
  ...
  5. Success:
     - [ ] miyo titles unchanged `[ref: PRD/AC F-55]` — [ ] lyt titles plain `[ref: PRD/AC F-55]`

- [ ] **T2.2 topic_clusters strip by param** `[activity: backend]` `[parallel: true]`
...
- [ ] **T2.3 reducer suffix + `conventions` block** `[activity: backend]`
  ...
  5. Success:
     - [ ] `conventions` block present (additive) `[ref: SDD/Modified wire]`
     - [ ] miyo reducer output otherwise unchanged `[ref: PRD/AC regression]`

- [ ] **T2.4 shared-ctx-builder `_MOC_NAME_RE` from suffix** `[activity: backend]` `[parallel: true]`
