...
| 2026-07-22 | Extension: Tomo-Editor wire channel | Hashi is adding a garden-audit review surface to its editor (renamed "Tomo Editor"), writing decisions into the JSON (machine/Hashi channel) — corrected two-channel model from Marcus: `.json` = Hashi channel, `.md` = human channel; Tomo already consumes the JSON when changed (`_is_wire_edited`→`build_from_wire`), else falls back to the `.md` (`build_from_report`). Additive wire fields (`schema_version` stays "1"): `decision.file_under` (orphan filing target), `decision.candidates:[{stem,score}]` (display-only LLM picks), `decision.suggest_requested` (editor request flag), top-level `approved` (JSON-side approve gate). Change-detection digest split: `compute_garden_audit_digest` hashes ONLY the apply-decision fields (`selected`/`repoint`/`replace`/`file_under`) so Tomo-written candidates never read as a user edit; the suggestions wire's `compute_payload_digest` is unchanged. `--suggest` writes `candidates` into the wire (SSoT with the markdown pick lists) and reads `suggest_requested`. `build_from_wire` reads `file_under` (explicit wins > scan candidate; candidates never auto-applied). inbox-triage gates garden-audit pickup on markdown `- [x] Approved` OR wire `approved:true`; `approved:true` forces the JSON path regardless of digest (all-default editor approval still applies fixes). Handoff-back: `_outbox/for-hashi/2026-07-22_tomo-to-hashi_garden-audit-wire-schema-and-answers.md`. |
...
| 2026-09-01 | Hashi gap closed — the example-driven bet paid off | The 2026-07-18 "Hashi gap" row above is retired. Hashi **0.18.0** (2026-07-25) shipped `resolve_dead_link` (commit `a18577c`) and `remove_up_link`, both registered in `src/actions/index.ts` and re-vendoring `instructions.schema.json`; verified present in Hashi **0.20.0**. Tomo's `resolve_dead_link` contract was implemented verbatim (bare/aliased/embed unlink + repoint, every-occurrence replace, skip-on-no-match) plus three hardenings: whole-target matching (`[[Old MOC]]` untouched when target is `MOC`), literal matching of regex-special-char targets, and frontmatter frozen verbatim. Consequence: the 2026-07-24 sequencing hold ("don't apply garden-audit docs with dead_link fixes until the executor ships") is **lifted** — parked rounds, incl. `2026-07-24_1055` (F06 aliased no-op), can proceed. Audit confirmed the hold was never encoded in Tomo: no feature flag, no min-Hashi-version pin, no exclusion of `dead_link` from `_FIXABLE` (`garden-audit.py:60`); the only apply gate is `suggest_pending` (`inbox-triage.py:697`), which is check-agnostic. Nothing to un-gate in code — docs hygiene only. Source: `_inbox/from-hashi/2026-07-28_hashi-to-tomo_garden-audit-executor-and-gate-shipped.md`. |
