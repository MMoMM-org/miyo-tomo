...
- [ ] **T4.1 Remove tag-prefix from the install wizard + config** `[activity: backend-api]`
...
- [ ] **T4.2 Delete dead `state-scanner.py`** `[activity: backend-api]` `[parallel: true]`
...
- [ ] **T4.3 Repoint statusline tag-access probe → frontmatter** `[activity: backend-api]`
...
- [ ] **T4.4 `/inbox` lifecycle regression guard** `[activity: validate]`
