...
> Updated: 2026-05-20 (F-47 Tomo lifecycle tags added from F-43 T6.2 live-validation findings; F-43 status moved to "launch blocked-by F-47").
...
| F-49 | `instruction-render.py` `resolve_stem_to_path`/`path_exists` latent bug | 2026-05-21 F-47 Phase 2 T2.3 implementation (SDD §Risks Known Technical Issues) | Could | `instruction-render.py:388/416` reference `KadoClient.resolve_stem_to_path()` and `KadoClient.path_exists()` — neither method exists on the current `KadoClient` API. This is a pre-existing latent bug (not introduced by F-47); F-47 T2.3 intentionally left it alone per the SDD §Risks/Known Technical Issues lock. When triggered (rare code path — only fires when an unresolved wikilink stem appears in the up:: preservation flow), the script will raise `AttributeError`. Fix: either implement the two methods on `KadoClient` (using `kado-search operation=listDir` + a stem-to-path scan) OR refactor the call sites to avoid them (compute the resolution inline using existing `kado-search byFrontmatter` with `tomo.doc_type=source`). Low-frequency code path; not blocking; surface when next live `/inbox` run hits the trigger. |
| F-50 | A2.5a stop-gate violates AC-5b.4 — exits unconditionally when MEDIA_COUNT > 0 | 2026-05-21 F-47 first live-run on Privat-Test | Should | `inbox-orchestrator.md` v0.10.1 A2.5a step exits 0 whenever any media file is in the inbox, even if (a) voice-transcriber reports `transcribed=0` (all already skipped), or (b) untagged manual `.md` notes also exist in the inbox. PRD AC-5b.4 explicitly says: "Given media files AND untagged manual `.md` notes both exist in inbox on the same run, When `/inbox` runs, Then transcription completes for media AND Pass-1 runs for the existing manual `.md` notes. Only the **newly produced transcripts** are deferred to the next run; existing notes are not gated by transcription." Current behaviour blocks the inbox forever when .m4a files sit alongside transcripts + manual notes. **Workaround**: move processed .m4a to `assets/` subdir (orchestrator's listDir is top-level only). **Proper fix**: after voice-transcriber returns its summary JSON, branch on `transcribed`: (i) `==0` → continue to A2.5b silently (no new transcripts produced); (ii) `>0` AND no other untagged manual .md exist → emit stop-gate message + exit; (iii) `>0` AND manual .md exist → continue to A2.5b but build a per-run skip-list of newly-transcribed stems (need to flow into A2.5c/A2.5e/Phase B's newSources filter). Touch points: `inbox-orchestrator.md` A2.5a (logic + STRICT block on the new branching), `inbox-discovery.py` (accept `--exclude-stems` flag OR orchestrator filters newSources post-discovery), tests for the three branches. SDD §Implementation Examples §Two-run gate (Feature 5b) needs a concrete walkthrough of the AC-5b.4 path. **First surfaced when**: 2026-05-21 manual /inbox run on Privat-Test got stuck in stop-gate loop with .m4a + their transcripts both in inbox. |
...
| F-52 | Voice-transcriber dispatch optimization — skip when prior summary indicates all media already transcribed | 2026-05-21 F-47 first live-run on Privat-Test | Could | Every `/inbox` run currently dispatches `voice-transcriber` (Phase 0a) whenever voice is enabled, even when the prior run's `tomo-tmp/voice/summary.json` shows `transcribed: 0, skipped: N` for the EXACT same set of media files and their .md siblings still exist. Cost: ~17.6k tokens per dispatch (measured 2026-05-21) for zero actual transcription work. **Proposed optimization**: extend `tomo-tmp/voice/summary.json` schema with a `last_seen` map: `{media_path: {mtime: int, transcript_path: str, transcript_mtime: int}}`. Before Phase 0a dispatches voice-transcriber, the orchestrator (a) listDir the inbox for media + .md files, (b) reads the prior summary if it exists, (c) compares: if every current media file has a matching `last_seen` entry whose `mtime` is unchanged AND the listed `transcript_path` still exists with the recorded `transcript_mtime` → **skip dispatch entirely**, set `voice_summary = {"transcribed": 0, "skipped": N, "reason": "all-cached"}` inline. (d) On any mismatch: dispatch normally, then update `last_seen` on return. Saves 17k tokens per re-run on a stable inbox. Touch points: `inbox-orchestrator.md` Phase 0a (cache-check logic before dispatch), `voice-transcriber.md` (update summary schema to emit `last_seen` entries), new test coverage for cache-hit / cache-miss / partial-mismatch. Spec impact: extend XDD 009 (voice-memo-transcription) with the caching contract OR scope to F-47 follow-up. |
