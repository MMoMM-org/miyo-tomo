...
## `merge_tomo_block_into_markdown()` — stamp a tomo block without a YAML round-trip

`lib/doc_frontmatter.merge_tomo_block_into_markdown(text, block)` inserts a `tomo:` block immediately after the opening `---` fence, leaving every existing frontmatter line **byte-identical** (inline arrays, quoting, Templater tokens, key order all preserved) — it does NOT `yaml.safe_load` + re-dump the note's own frontmatter. Use it to stamp rendered notes/MOCs (#108); never regex-edit or full-round-trip rendered-note frontmatter. Raises `FrontmatterMergeError` on an unclosed fence, a pre-existing top-level `tomo:` key, or missing PyYAML — callers (`instruction-render.py` render loop) catch it and write the note unstamped (fail-safe: worst case is triage re-ingestion, never a corrupted note). Lib version 0.5.0.

<!-- 2026-07-20 -->

## `KadoClient.write_note`/`write_file` raise generic `KadoToolError` on an `expectedModified` mismatch — NOT `KadoConcurrencyError`

Only `write_frontmatter` re-raises the optimistic-concurrency failure as the typed `KadoConcurrencyError` (`kado_client.py:437-440`). `write_note`/`write_file` go straight through `_call_tool` → `_parse_rpc_response` (`kado_client.py:843`), which raises the **generic `KadoToolError`** — the mismatch is only distinguishable by the `"expectedModified"` + `"mismatch"` text in the message. So any caller detecting a write-path concurrency conflict must check BOTH the typed exception AND the message signature (see `kado-write-file.py._is_concurrency_conflict`, spec 030). Related: `kado-write-file.py` now auto-resolves the guard on overwrite (fetches the target's current `modified` via `read_note`/`read_file`, `KadoNotFoundError` → new-file/no-guard) so the garden-audit suggest re-upload — the first overwrite-existing use of the script — works. Kado requires `expectedModified` to overwrite an existing note; a new file needs no guard.

<!-- 2026-09-04 -->
- **Live-validating `/garden-audit` needs two container commands and one config intervention — the repo session can run neither command.** `/explore-vault` and `/garden-audit` live in `tomo-instance/.claude/commands/`; they run in the Tomo Docker session, so a repo-side dev session can only prepare and measure, never invoke them. Full procedure: (1) `./scripts/update-tomo.sh --instance <name> --dry-run` to see the plan, then `--yolo` to sync (the bare form dies at the voice prompt without copying, and an unchanged `# version` ships nothing — grep the instance after); (2) **user** runs `/explore-vault` to rebuild `config/moc-structure-cache.yaml`; (3) narrow the exclusion config; (4) **user** runs `/garden-audit`; (5) restore. The intervention at (3) is not optional: `tomo-instance/config/garden-audit-exclusions.yaml` carries a temporary `Atlas/` rule with `checks: all` (until 2026-10-19), and the cache's `scope_paths` are both under `Atlas/` — **329 of 359 entries and all 41 broken ones sit inside it**, so a live run reports *zero* broken-parent findings and proves nothing. Measured: 41 findings without exclusions, 0 with them. Spec 032 hit the same wall and called it "the one config intervention the last step needs". Narrow the rule to its non-target checks, run, then restore byte-for-byte and verify with `filecmp`. Artifacts from the run land in `tomo-instance/tomo-tmp/` (`garden-audit-report.md`, `-doc.json`, `-wire.json`) and are readable from the host — no need to ask the user to paste the report.

<!-- 2026-09-07 -->
- **A same-size, same-second revert reuses the stale `.pyc` — so "reverting proves red" can silently prove nothing.** Found during spec 034 T5.4 while proving a composition test red by swapping two call blocks in `instruction-render.py`. The swap left the file byte-size-identical with an mtime in the same second, so CPython's source-timestamp invalidation (size + mtime to one-second granularity) considered the cached bytecode current and reran the *old* code. The restored source kept "failing", and the failure was the cache, not the logic. Cleared with `find . -name __pycache__ -type d -exec rm -rf {} +`. This matters more than a normal gotcha because revert-to-prove-red is the verification method this repo leans on hardest — every task in spec 034's phase 5 was validated that way, and a stale `.pyc` makes a green revert look like a red one or vice versa. Any revert that swaps or reorders blocks (rather than adding or deleting lines) is in the danger zone: those are exactly the edits that preserve file size. Clear `__pycache__` before trusting the result, or touch the file to a later mtime.
