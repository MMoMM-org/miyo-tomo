...
- [x] **ADR-13 Proposal coherence: drop per-note orphans from /moc-propose, dedup overlapping clusters, exclusion tags** (PRD F8 — added 2026-06-07 after proposal-doc review).
  - **Problem (live, doc `2026-06-07_0850`):** (P1) MOC01 listed 9 notes twice — `candidate_stems` not deduped. (P2) 22 notes appeared in BOTH a proposed cluster AND the per-note orphan section — the scan cluster path and the case-(a) orphan pass (ADR-7) both consume the same `up_state==absent` set with no dedup. (P3) the orphan cap (ADR-12) made the two views asymmetric. (P4) MOC05⊆MOC02 (100%) and MOC04 97%⊆MOC03 survived because `phase6_dedupe` only compares cluster TOPICS vs EXISTING MOCs, never proposals against each other on MEMBERS.
  - **Decision:**
    1. **D1 — `/moc-propose` = cluster→new-MOC only.** Remove the per-note orphan section (`### Oxx`) from the cluster proposal-doc. The orphan-section renderer + ADR-12 cap/overflow stay, used ONLY by `check:moc-uplinks` (MOC orphans). Rationale: a note without `up::` does not warrant a MOC — worst case one MOC per note. Interactive note-orphan handling is deferred to **Garden-Audit (#30)**.
    2. **D2 — note-orphan flow → #30** (Garden-Audit): detect → review + file / link-to-existing, interactive, reusing 021's cache + `lib/up_parse` + `lib/orphan_link`. Per-note `create_new` deprecated; the orphan-apply wiring (parse ticked choices → instructions) lives there, not in moc-propose.
    3. **D3 — inter-cluster member-overlap dedup.** A NEW pass over the PROPOSED clusters: when ≥80% of a smaller cluster's members are also in a larger proposed cluster, drop the smaller (keep the larger), recording it in `duplicates_skipped`. Complements `phase6_dedupe` (topics-vs-existing); does not replace it.
    4. **D4 — dedup `candidate_stems`** within a cluster (each note at most once).
    5. **B-moc — `MiYo/Tomo/exclude/moc` = audit-only:** skip in `emit_orphan_suggestions(kinds=("moc",))`; the MOC STAYS in the cache + link-candidate pool. Root MOCs (`000 Index`) carry the tag (user-applied) — no root-path heuristic. Filter is in the orphan pass, NOT in `moc_scan` discovery.
    6. **B-note — `MiYo/Tomo/exclude/note`:** filter at the scan candidate source (`_handle_scan`, cache-based) so a tagged note is never clustered into a proposed MOC. Scoped live runs: tag-filter deferred (the user scoped deliberately).
    7. **Dependency:** the cache builder must populate each entry's `tags` (current dump shows `tags: []`); both exclude filters read `tags`. Hardwired tag constants (not user-configurable).
  - **Trade-offs:** dropping per-note orphans from moc-propose means the only orphan visibility from `/moc-propose` is `check:moc-uplinks` (MOCs) until Garden-Audit ships — acceptable, the note-orphan flow needs the interactive UX #30 provides. The ≥80% dedup could merge two genuinely-distinct themes that happen to share most notes — accepted (keeping the larger loses nothing the smaller had).
  - **Relationship to ADR-7/11/12:** ADR-7's orphan pass survives but is consumed only by `check:moc-uplinks`; ADR-12's cap/overflow/ordering apply there. ADR-11's scan candidate model is unchanged except B-note now filters tagged notes from the candidate source.
  - User confirmed: **Yes (2026-06-07)** — drop orphan section (Option C), ≥80% member-overlap dedup, exclude tags `MiYo/Tomo/exclude/{moc,note}`, orphan flow → #30.
