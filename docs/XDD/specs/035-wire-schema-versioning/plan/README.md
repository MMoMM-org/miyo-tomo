...
| 2026-09-10 | T3.2 | **F5-AC2 narrowed to the document Tomo owns.** Only `instructions.schema.json` (the producer copy) takes a distinct `$id` and a role-stating title/description. `hashi-instructions.schema.json` is left byte-identical. | F5-AC2 asks that *each* of the two documents state its role. Measured obstacle: `hashi-instructions.schema.json` is a **verbatim mirror of Hashi's live wire schema**, refreshed by pulling their file from `raw.githubusercontent.com` (`docs/ai/memory/decisions.md:7`). A Tomo-authored role line in it (1) makes the file stop being verbatim, which is the property the whole parity guard rests on, and (2) is **destroyed by the next re-vendor** — which this very spec performs at T4.2/T4.4. An annotation that a routine maintenance step deletes is worse than none, because it reads as present while being absent. The `$id` half is not a choice either: Hashi's own file declares `https://miyo.tomo/schemas/instructions.schema.json`, so the mirror must keep it and the producer copy must be the one that moves — which is what ADR-6 says regardless. T3.3's check ("a reader given either schema alone can tell which one they hold") still passes: the producer copy self-identifies, and the other is identified by carrying the canonical `$id` with no producer annotation. | Owner, 2026-09-10 |
...
- [x] [Phase 3: Version integrity and document identity](phase-3.md)
- [x] [Phase 4: The CLI, the consumer copies, and closing the live drift](phase-4.md)
